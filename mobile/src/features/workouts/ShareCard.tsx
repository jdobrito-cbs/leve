...
export const ShareCard = forwardRef<
  View,
  { workout: Workout; photoUri?: string | null; onMapReady?: () => void }
>(function ShareCardImpl({ workout: w, photoUri, onMapReady }, ref) {
  const { colors } = useTheme();
  const route = w.route ?? [];
        ...
        <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: spacing.md }}>
          <View style={{ flexDirection: 'row', gap: spacing.md, alignItems: 'flex-start' }}>
            <View style={{ width: 86, height: 86, borderRadius: 12, overflow: 'hidden' }}>
              <RouteTiles route={route} w={86} h={86} strokeWidth={3} onReady={onMapReady} />
            </View>
  ...
  return (
    ...
    >
      ...
      <RouteTiles route={route} w={CARD_W} h={MAP_H} onReady={onMapReady} />
